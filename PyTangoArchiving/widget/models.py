...
from taurus.qt.qtcore.model.taurusmodel import TaurusBaseTreeItem, TaurusBaseModel, TaurusBaseProxyModel
from taurus.core.tango.tangodatabase import *
from taurus.core.tango.tangodatabase import ( TangoDatabase, TangoDatabaseCache, 
    TangoServInfo, TangoDevTree, TangoServerTree, TangoDevClassInfo )
...
class TaurusArchivingDatabase(TangoDatabase):
    ...
    def __init__(self, host=None, port=None, parent=None):
        ...
        self._reader = PyTangoArchiving.Reader()
        self._archived_devs = None
        
    @fn.objects.Cached(depth=10,expire=120.)
    def get_archived_devices_list(self,active=True,attrs=None):
        attrs = attrs or self.get_archived_attributes_list(active)
        self._archived_devs = sorted(set(
            fn.tango.get_normal_name(a).rsplit('/',1)[0] for a in attrs)) 
